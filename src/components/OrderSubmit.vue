  ...
  <div>
    ...
    <div class="container" v-if="!loading">
      ...
      <div class="order-info" v-if="receiptway.storeDeliver">
        ...
        <div class="label-message">
          <div class="label-list">
            <label>自提人</label> <input class="inputs" type="text" v-model='formData.name' @focus="focusMethod('name')" @blur="blurMethod('name')" placeholder="请填写自提人姓名(必填)">
          </div>
          <div class='err-class tright'v-if="validator.name.errIsShow">{{validator.name.errText}}</div>
          <div class="label-list">
            <label>联系电话</label> <input class="inputs" type="text" v-model='formData.telphone' @focus="focusMethod('telphone')" @blur="blurMethod('telphone')" placeholder="请填写自提人联系方式(必填)">
          </div>
      ...
      <div class="order-detail">
        ...
        <ul v-if="goodsInfo.buyWay!== 'cart'">
          <li>
            ...
            <div class="price">
              <p>{{this.goodsInfo.selectedSpecGood.price | price}}</p>
      ...
      <div class="order-pay">
        ...
        <div>
          <div class="selected" :class="[payWay==='online' ? 'selected1' : 'selected']"></div>
      ...
      <div class="order-submit ">
        <p class="">合计：<span>{{ totalPrice|price }}</span></p>
        <button class="btn" @click="orderSubmit">提交订单</button>
      </div>
    </div>
     <!--预约时间选择-->
        <select-time 
        :time-show="appointTime.timeShow" 
        :selected-date="appointTime.selectedDate"
        :selected-time="appointTime.selectedTime" 
        :date="appointTime.date"    
        :time="appointTime.time" 
        v-on:close="timeClose" 
        ...
        if (this.goodsInfo.buyWay !== "cart") {
          ajax.postDataToApi({
            ...
            if(this.goodsInfo.buyWay === "score"){
              location.href = `/#/site/score-success/${response.data.body.id}`
            }
          })
        }
      }
